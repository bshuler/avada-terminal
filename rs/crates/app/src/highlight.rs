...
const JS: Syntax = Syntax {
    ...
    keywords: &[
        ...
        "protected", "public", "readonly", "require", "return", "satisfies", "set", "static",
        "super", "switch", "this", "throw", "true", "try", "type", "typeof", "undefined",
        "var", "void", "while", "yield",
    ],
    types: &["number", "string", "symbol", "bigint", "never", "unknown", "object"],
    ..DEFAULT
};
...
const SWIFT: Syntax = Syntax {
    keywords: &[
        ...
        "init", "inout", "internal", "is", "let", "mutating", "nil", "open", "operator",
        "private", "protocol", "public", "repeat", "required", "return", "self", "Self",
        "static", "struct", "subscript", "super", "switch", "throw", "throws", "true", "try",
        "typealias", "var", "weak", "where", "while",
    ],
    types: &["Bool", "Int", "Double", "Float", "String", "Character", "Void", "Any"],
    ..DEFAULT
};
...
const KOTLIN: Syntax = Syntax {
    keywords: &[
        ...
        "enum", "expect", "external", "false", "final", "finally", "for", "fun", "get",
        "if", "import", "in", "infix", "init", "inline", "inner", "interface", "internal",
        "is", "lateinit", "null", "object", "open", "operator", "out", "override", "package",
        "private", "protected", "public", "reified", "return", "sealed", "set", "super",
        "suspend", "this", "throw", "true", "try", "typealias", "val", "var", "vararg",
        "when", "where", "while",
    ],
    types: &["Boolean", "Byte", "Short", "Int", "Long", "Float", "Double", "Char", "String", "Unit", "Any"],
    ..DEFAULT
};
...
const JAVA: Syntax = Syntax {
    keywords: &[
        ...
        "protected", "public", "record", "return", "sealed", "static", "super", "switch",
...
const DART: Syntax = Syntax {
    keywords: &[
        ...
        "finally", "for", "get", "if", "implements", "import", "in", "is", "late",
    ...
    types: &["bool", "double", "int", "num", "String", "List", "Map", "Set", "Future", "Stream"],
    ..DEFAULT
};
...
const SQL: Syntax = Syntax {
    line_comment: &["--"],
    keywords: &[
        "ALTER", "AND", "AS", "ASC", "BY", "CASE", "CREATE", "DELETE", "DESC", "DISTINCT",
...
const CSS: Syntax = Syntax {
    keywords: &["@import", "@media", "@mixin", "@include", "@use", "@extend", "@keyframes", "@function", "@return", "!important"],
    camel_is_type: false,
    ..DEFAULT
};
...
const SLINT: Syntax = Syntax {
    quotes: &['"'],
    keywords: &[
        "animate", "callback", "component", "export", "for", "function", "global", "if",
        "import", "in", "in-out", "inherits", "out", "property", "private", "public",
        "pure", "return", "states", "struct", "enum", "transitions", "true", "false",
    ],
    types: &["angle", "bool", "brush", "color", "duration", "float", "image", "int", "length",
             "percent", "physical-length", "string"],
    ..DEFAULT
};
...
fn scan(line: &str, syn: &Syntax, carry: Carry) -> (Vec<Span>, Carry) {
    ...
    while i < n {
        ...
        if sigil || c.is_alphabetic() || c == '_' {
            ...
            while i < n && (ch[i].1.is_alphanumeric() || ch[i].1 == '_' || (ch[i].1 == '-' && syn.markup)) {
                i += 1;
            }
...
mod tests {
    ...
    const CORPUS: &[(&str, &str)] = &[
        ...
        ("rs", "fn f<'a>(v: &'a str) -> Option<&'a str> { v.get(0..1) }"),
        ("rs", "let n = 0xff_u32 + 1.5e3 - 2;"),
        ("ts", "const t = `a ${x} b`; // 100% _fine_"),
        ("py", "def f(a=1, *args, **kw):  # trailing"),
        ("py", "    return {'k': \"v\"}"),
        ("sh", "if [ -n \"$1\" ]; then echo \"hi ~ | & <>\"; fi"),
        ("html", "<div class=\"a-b\" data-x='1'>text &amp; more</div>"),
        ("sql", "SELECT a.b FROM t WHERE x LIKE '%y%' -- note"),
        ("css", "@media (min-width: 40px) { .a > .b { color: #fff !important; } }"),
        ("go", "\t\tif err != nil { return fmt.Errorf(\"x: %w\", err) }"),
        ("rs", ""),
        ("rs", "        "),
    ];
    ...
    fn markup_reproduces_the_source_line_exactly() {
        ...
        for (ext, line) in CORPUS {
            ...
            let want = detab(line);
            assert_eq!(got.trim_end(), want.trim_end(), "{ext}: {line:?} became {:?}", m[0]);
        }
    }
    ...
    fn multi_line_forms_carry() {
        ...
        assert!(py[1].contains(&strink), "still inside the triple quote: {:?}", py[1]);
        assert!(!py[2].contains(&strink), "it closed on the line before: {:?}", py[2]);
    }
    ...
    fn a_lifetime_is_not_a_string() {
        ...
        assert!(!m[0].contains(&strink), "no string ink on a lifetime: {:?}", m[0]);
        let m = markup_lines(&["let c = 'x';"], "rs", &p).unwrap();
        assert!(m[0].contains(&strink), "a char literal is a string: {:?}", m[0]);
    }
    ...
    fn only_known_extensions_are_source() {
        for ext in ["rs", "ts", "tsx", "py", "html", "sh", "swift", "kt", "scss", "slint"] {
