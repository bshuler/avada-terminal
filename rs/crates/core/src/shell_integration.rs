...
pub fn shell_integration_dir() -> PathBuf {
    ...
    let candidates = [
        ...
        // FHS system install (deb/rpm): the binary lands in <prefix>/bin/avada and the
        // scripts in <prefix>/share/avada/resources/shell-integration (an arch-independent
...
pub fn integration_for(shell: &str, dir: &Path) -> Option<Integration> {
    match classify(shell) {
        ...
        ShellKind::Zsh => {
            ...
            // ZDOTDIR via AVADA_ZDOTDIR_ORIG) and then source hp-init.sh. Mechanism
            ...
            if let Ok(orig) = std::env::var("ZDOTDIR") {
                if !orig.is_empty() {
                    env.push(("AVADA_ZDOTDIR_ORIG".to_string(), orig));
                }
            }
            Some(Integration {
