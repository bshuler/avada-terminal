//! `avada-table`: a tier-5 data viewer for Avada Terminal.
//!
//! This is a *panel type* that ships as a module. It claims `.csv`, `.tsv` and `.tab` in
//! its manifest; the host routes a matching file to its `table` surface and cues it with a
//! `doc.open` naming the surface and the path. The module then does the whole of what the
//! built-in table view did: read the file through `host.fs.read`, parse it with the shared
//! [`avada_csv_parse`] crate — the same parse the app's built-in view runs, so the two
//! cannot drift — and hand the host a [`Doc`] of one dense [`Block::Table`] through
//! `host.doc.set`. The host measures the columns and paints the monospace grid; the reader
//! only reads. **The module owns the records; the host owns the pixels.**
//!
//! `avada-table --manifest` prints the embedded `avada.toml` and exits.
...
fn run() -> Result<(), avada_module_sdk::client::ClientError> {
    eprintln!("avada-table: this module needs the Unix socket transport (AVADA_MODULE_FD)");
    Ok(())
}
...
fn run() -> Result<(), avada_module_sdk::client::ClientError> {
    ...
    // A data viewer draws a document and takes no keys back, so it serves the required set
    // and none of the grid optionals: a tier-5 surface that answers no keystroke is a doc,
    // not a grid.
    ...
    ///
    /// Every way the *read* can fail is something the reader did — a path outside the
    /// workspace, a missing file, a binary file — not a protocol fault, so each becomes a
    /// one-block [`Doc`] the reader sees in the pane rather than an error that tears the
    /// module down. A file that reads but does not parse as a table is not an error either:
    /// [`parse_table`] embeds that as a notice block of its own.
    fn render<R: Read, W: Write>(
        ...
        let blocks = match read_text(conn, path) {
            Ok(text) => parse_table(&text, delimiter_for(Path::new(path))),
    ...
    /// The file's text, or the notice to show in its place. A non-UTF-8 file has no table to
